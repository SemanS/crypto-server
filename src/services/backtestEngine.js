...
async function runBacktest({
    ...
    let decisionData;
    if(decisionTF === '1m') {
      decisionData = min1Data;  // pre 1m intervaly používame min1Data
    ...
    for(let i = startIdx; i < decisionData.length; i++) {
      ...
      const decisionTime = decisionCandle[0]; // Toto je "openTime" pre daný 1m interval
      ...
      let summaryRow;
      // Ak ešte nie je otvorená pozícia a prichádza signál, otvárame pozíciu:
      if(!positionOpen && (gptSignal === 'BUY' || gptSignal === 'SELL')) {
        ...
        if(i === startIdx) {
          // Pre prvý interval použijeme close cenu priamo z decisionCandle[4]
    ...
    // Forced closure – vykonáme len ak pozícia zostáva otvorená.
    if(positionOpen) {
      ...
      if(lastCloseTimeSent === forcedCloseTimeStr) {
        console.log(`[BacktestEngine] Forced closure update s closeTime ${forcedCloseTimeStr} už bol odoslaný, preskakujem.`)
      } else {
